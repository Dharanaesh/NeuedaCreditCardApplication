...
public class TransactionReports implements TransactionReportsRepo{
    ...
    @Override
    public List<ProfessionSales> getTotalTransactionsByProfession(){
        GroupOperation groupByStateTransactions = group("Job").sum("amt").as("sales");
        MatchOperation allStates = match(new Criteria("Job").exists(true));
        ProjectionOperation includes = project("sales").and("Job").previousOperation();
        SortOperation sortBySalesDesc = sort(Sort.by(Sort.Direction.DESC,"sales"));
        SampleOperation randomSample = Aggregation.sample(10);
        Aggregation stateAggregation = newAggregation(allStates, groupByStateTransactions, sortBySalesDesc, includes,randomSample);
        AggregationResults<ProfessionSales> groupResults = mongoTemplate.aggregate(stateAggregation, "transaction", ProfessionSales.class);
        List<ProfessionSales> result = groupResults.getMappedResults();
        return result;
